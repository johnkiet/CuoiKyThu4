...
<?php
	$product = new product();
	if(isset($_GET['update_type_slider']) && isset($_GET['type'])){
		$id = $_GET['update_type_slider'];
		$type = $_GET['type'];
		...
		?>
			...
			<tbody>
				...
				<tr class="odd gradeX">
					...
					<td><img src="upload/<?php echo $result_slider['sliderImage'] ?>" height="120px" width="400px"/></td>			
					<td>
						<?php
						if($result_slider['type']==1){
						?>
							<a href="?update_type_slider=<?php echo $result_slider['sliderId']?>&type=0" >On</a>
						<?php	
						} else { 
						?>
							<a href="?update_type_slider=<?php echo $result_slider['sliderId']?>&type=1" >Off</a>
						<?php	
						}
						?>
					</td>	
